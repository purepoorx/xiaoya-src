...
if [[ -s /data/strm_dir.txt ]] || [[ -s /data/local_dir.txt ]]; then
	local_dir=$(head -n1 /data/local_dir.txt | tr -d '\r')
    sqlite3 /opt/alist/data/data.db <<EOF
...
d="xiaoya"
e=$(head -n1 /docker.version)
curl -s --retry 3 --max-time 10 -X POST http://quark-uc.xiaoya.pro:8099/save  -F "device_info=$d"  -F "device_id=$e" 
