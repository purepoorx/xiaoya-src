...
if [[ -f /data/115share_list.txt ]] && [[ -s /data/115share_list.txt ]]; then
    ...
    if [ ! -z "$line" ]; then
        ...
        pwd=$(echo $line |cut -f4 -d" ")
        sqlite3 /opt/alist/data/data.db <<EOF
INSERT INTO x_storages VALUES($id,"/🏷️ 我的115分享/$mount_path",0,'115 Share',1440,'work','{"cookie":"xxx","root_folder_id":"$root_folder_id","qrcode_token":"","qrcode_source":"linux","page_size":20,"limit_rate":2,"share_code":"$share_id","receive_code":"$pwd"}','','2022-09-29 20:14:52.313982364+00:00',0,'name','ASC','front',0,'native_proxy','');
EOF
    fi
    done
fi
