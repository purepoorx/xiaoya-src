...
for base_url in "${base_urls[@]}"; do
    remote_ver=$(curl -s --retry 3 --max-time 5 --ipv4 ${base_url}/version.txt 2>/dev/null | grep -e '^[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}$')
...
if [ "$(printf '%s\n' "$local_ver" "$remote_ver" | sort -V | head -n1)" != "$remote_ver" ] || [ ! -f "${data_dir}/tvbox.zip" ] || [ ! -f "${data_dir}/update.zip" ] || [ ! -f "${data_dir}/index.zip" ]; then
    ...
    if [ "$success" = true ]; then
        for base_url in "${base_urls[@]}"; do
            if curl -s --retry 3 --max-time 10 --ipv4 --insecure -fsSL -o "${data_dir}/${tvbox_zip_bak}" ${base_url}/tvbox.zip >/dev/null 2>&1 && unzip -t "${data_dir}/${tvbox_zip_bak}" >/dev/null 2>&1; then
    ...
    if [ "$success" = true ]; then
        for base_url in "${base_urls[@]}"; do
            if curl -s --retry 3 --max-time 10 --ipv4 --insecure -fsSL -o "${data_dir}/${update_zip_bak}" ${base_url}/update.zip >/dev/null 2>&1 && unzip -t "${data_dir}/${update_zip_bak}" >/dev/null 2>&1; then
    ...
    if [ "$success" = true ]; then
        for base_url in "${base_urls[@]}"; do
            if curl -s --retry 3 --max-time 10 --ipv4 --insecure -fsSL -o "${data_dir}/${index_zip_bak}" ${base_url}/index.zip >/dev/null 2>&1 && unzip -t "${data_dir}/${index_zip_bak}" >/dev/null 2>&1; then
    ...
    if [ "$success" = true ]; then
        for base_url in "${base_urls[@]}"; do
            if curl -s --retry 3 --max-time 10 --ipv4 --insecure -fsSL -o "${data_dir}/${a115share_txt_bak}" ${base_url}/115share_list.txt >/dev/null 2>&1 && cat "${data_dir}/${a115share_txt_bak}" | grep -q "电影"; then
    ...
    if [ "$success" = true ]; then
        for base_url in "${base_urls[@]}"; do
            if curl -s --retry 3 --max-time 10 --ipv4 --insecure -fsSL -o "${data_dir}/${version_txt_bak}" ${base_url}/version.txt >/dev/null 2>&1 && cat "${data_dir}/${version_txt_bak}" | grep -q -e '^[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}$' ; then
