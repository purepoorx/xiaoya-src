...
async function fetchEmbyFilePath(itemInfoUri, mediaSourceId) {
    try {
        const res = await fetch(itemInfoUri, { max_response_body_size: 365535 });
        if (res.ok) {
...
async function fetchXYApi(xyurl) {
    try {                                 
        const res = await fetch(xyurl, { headers: { 'Content-Type': 'application/json;charset=utf-8' }, max_response_body_size: 365535} );
        if (res.status == 302) { 
